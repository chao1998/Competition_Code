...
class OptimizedModel:
    def __init__(self):
        self.train_data_path = "C:/Aveni-HKU/firstround-crypto-pred/train_data"
        self.submission_id_path = "C:/Aveni-HKU/firstround-crypto-pred/submission_id.csv"
    ...
    def parse_required_id_format(self, sample_id):
        """Parse the required ID format to understand the expected datetime format"""
        try:
            if '_' in sample_id:
                parts = sample_id.split('_')
                datetime_part = parts[0]
                symbol_part = parts[1] if len(parts) > 1 else None
                
                # Try to parse the datetime part
                try:
                    # Try format: "2023-02-26 10:45:00"
                    parsed_dt = pd.to_datetime(datetime_part, format='%Y-%m-%d %H:%M:%S')
                    return True, '%Y-%m-%d %H:%M:%S', symbol_part
                except:
                    pass
                
                try:
                    # Try format: "20230226104500"
                    parsed_dt = pd.to_datetime(datetime_part, format='%Y%m%d%H%M%S')
                    return True, '%Y%m%d%H%M%S', symbol_part
                except:
                    pass
                ...
                return False, None, symbol_part
            
            return False, None, None
            
        except Exception as e:
            print(f"Error parsing ID format: {e}")
            return False, None, None
    ...
    def train(self, target_features, all_features):
        ...
        print(f"Submission ID file loaded: {len(df_submission_id)} IDs")
        
        # Analyze the required format
        sample_required_ids = df_submission_id['id'].head(5).tolist()
        print(f"Sample required IDs:")
        for i, id_str in enumerate(sample_required_ids):
            print(f"  {i+1}: {id_str}")
        
        # Parse the format
        success, datetime_format, sample_symbol = self.parse_required_id_format(sample_required_ids[0])
        
        if not success:
            print("Could not parse required ID format! Using default format...")
        ...
        # Create submission ID using the detected format
        print(f"Creating submission IDs using format: {datetime_format}")
        ...
        if len(overlap_ids) == 0:
            print("WARNING: Still no ID overlap!")
            print("This suggests the symbols or datetime ranges don't match.")
            print("Creating submission with available predictions...")
            
            # Check for symbol overlap
            required_symbols = set()
            available_symbols = set(df_submit['symbol'].tolist() if 'symbol' in df_submit.columns else [])
            
            # Extract symbols from required IDs
            for req_id in list(required_ids)[:100]:  # Check first 100
                if '_' in req_id:
                    symbol = req_id.split('_')[1]
                    required_symbols.add(symbol)
            
            symbol_overlap = required_symbols & available_symbols
            print(f"Required symbols (sample): {list(required_symbols)[:10]}")
            print(f"Available symbols (sample): {list(available_symbols)[:10]}")
            print(f"Symbol overlap: {len(symbol_overlap)}")
            
            # Create submission with all required IDs, filling missing with 0
            df_submit_final = pd.DataFrame({
                'id': list(required_ids),
                'predict_return': 0.0
            })
            
            # Update with available predictions
            df_submit_dict = df_submit.set_index('id')['predict_return'].to_dict()
            df_submit_final['predict_return'] = df_submit_final['id'].map(df_submit_dict).fillna(0.0)
            
            non_zero_final = np.count_nonzero(df_submit_final['predict_return'])
            print(f"Final non-zero predictions: {non_zero_final}")
        ...
        # Save submission
        df_submit_final.to_csv('submit.csv', index=False)
        print("Submission saved to submit.csv")
...
if __name__ == '__main__':
    ...
    train_data_path = "C:/Aveni-HKU/firstround-crypto-pred/train_data"
